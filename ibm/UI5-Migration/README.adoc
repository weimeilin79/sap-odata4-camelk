...
Since the UI5 only runs on 127.0.0.1(which means only accessible by localhost), we need to create a tunnel

Option 1. Using ngrok, run this inside the container.
...
Option 2. instead of using ngrok, we can deploy a container with an NGINX proxy server in the same pod where the application runs, this way it will redirect the external traffic to the application and we will have a permanent end point
...
oc delete route socreate
oc expose dc socreate --port=8081
